...
%Code to analyze static MAE v6 expt
%Loic Daumail - 03/14/2024
...
for i =1:length(names)
    ...
    exptDat = load(strcat(fileNames(5).folder,'/',fileNames(5).name));
    exptDat = exptDat.ex;
...
mkdir(plotdir);
saveas(gcf,strcat(plotdir, sprintf('MAE_duration_%s_4deg_1min.png',version)));
...
singleBarPlotSD(yval,conds,respType, ylab, [0 100])

plotdir = strcat('/Users/loicdaumail/Documents/Research_MacBook/Tong_Lab/Projects/motion_after_effect/anal_plots/');
mkdir(plotdir);
saveas(gcf,strcat(plotdir, sprintf('MAE_percent_type_%s_4deg_1min.png',version)));
