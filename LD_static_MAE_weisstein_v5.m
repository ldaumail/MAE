function LD_static_MAE_weisstein_v5(subject, session, debug)
...
ex.blockLength = 3; %120; %ex.trialFixation+ ceil(ex.stimDur*ex.stimsPerBlock);           % in seconds
...
for c = 1:length(ex.condShuffle)
    ...
    while n <= length(ex.longFormBlocks) %true
        ...
        if nnz(find(ex.longFormStimOnSecs(n)))
            ...
            % stim
            if contains(condName, 'Ct')
                if contains(condName, 'Right') %Based on top grating
                    Screen('DrawTexture', w, ex.rectSWaveID(f),[],ex.rectLRect);
                    Screen('DrawTexture', w, ex.rectSWaveID(f),[],ex.rectRRect);
                elseif contains(condName, 'Left')
                    Screen('DrawTexture', w, ex.rectSWaveID(end-(f-1)),[],ex.rectLRect);
                    Screen('DrawTexture', w, ex.rectSWaveID(f),[],ex.rectRRect);
                end
            else
                ...
                elseif contains(condName, 'Left')
                    Screen('DrawTexture', w, ex.rectSWaveID(end-(f-1)),[],ex.rectLRect);
                    Screen('DrawTexture', w, ex.rectSWaveID(end-(f-1)),[],ex.rectRRect);
        ...
        if length(ex.longFormBlocks(1:n))/60 == ex.blockLength+ex.testLength %&& c ~= length(ex.condShuffle) %(cnt/2 == 1 && GetSecs-time >= ex.blockLength+ex.testLength) && c ~= length(ex.condShuffle)
            ...
            if c ~= length(ex.condShuffle)
                ...
                Screen(w, 'Flip', 0);
                [~,~,~] =KbWait(deviceNumber,2);
                Screen('FillRect', w, ex.stim.backgroundLum(1,:));
