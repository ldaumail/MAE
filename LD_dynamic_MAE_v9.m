...
ex.stim.gaborWDeg = 6; 
ex.stim.distFromFixDeg = 2;%3;%2; %1.5 %each grating edge 1.5 deg horizontal away from fixation (grating center 6 deg away)
...
for n = 1:length(phases1)
    ex.testStim(:,:,n) = makeCounterPhasingGrating(ex.rawProbeHeight,ex.rawProbeWidth,ex.stim.spatialFreqDeg,...
        ex.stim.orientation,spphase,ex.test.contrastOffset,ex.test.contrastMultiplicator, ex.test.tmpphase(n),...
        ex.ppd);
