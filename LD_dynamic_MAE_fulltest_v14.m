function LD_dynamic_MAE_fulltest_v14(subject, session, debug)
...
ex.test.gaborWDeg = ex.stim.gaborWDeg;%4;%
...
ex.conds = {'vLowContFullRight','vLowContFullLeft'}; 
ex.repsPerRun = [12 12];              % repetitions of each condition per run
...
%%%% screen
ex.backgroundColor = [127.5  127.5  127.5];%[108.3760 108.3760 108.3760];%;  % color based on minimum gating luminance 
ex.fontSize = 26;
...
ex.test.tmpphase = ex.test.oscillation1.*90+ex.test.spatialPhase1; %./ex.stimDur-2*pi*flipTimes./ex.stimDur make it oscillatory
...
for c = 1:length(ex.condShuffle)
    ...
    while n <= length(ex.longFormBlocks) %true
        ...
        f = f+1;
        n = n+1;
        if f == ex.flipsPerSec/ex.stim.cycPerSec
            f = 1;
        end
    end
