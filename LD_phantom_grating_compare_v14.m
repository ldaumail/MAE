function LD_phantom_grating_compare_v14(subject, session, debug)
...
ex.test.orientation = ex.stim.orientation; %[90 180];                                                % in degrees
...
ex.conds = {'vLowContPhRight','vLowContPhLeft','vLowContPhCtRight','vLowContPhCtLeft'}; 
...
for i =1:ex.repsPerRun
    ...
    ex.locShuffle = [ex.locShuffle, Shuffle([1,2,1,2])]; %2 locations (left and right)
end
...
% grating y locations
yLT = yc- ex.stim.distFromFix;
yLB = yc+ ex.stim.distFromFix;
%right grating
xR = xc + rect(3)/4 ;

yRT = yc- ex.stim.distFromFix;
yRB = yc+ ex.stim.distFromFix;
...
while(1) %n <= length(ex.trialFlips)
    ...
    if ex.locShuffle(c) == 1
        ex.rectLRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xL,yLT);
        ex.rectRRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xL,yLB);
        ex.rectCRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.gapSize],xR,yc);
    elseif ex.locShuffle(c) == 2
        ex.rectLRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xR,yRT);
        ex.rectRRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xR,yRB);
