...
[ttestMean(1), Pval(1),~,Stats(1).stats] = ttest(yvar(:,4,3),yvar(:,5,3));%phantom low vs phantom control low
[ttestMean(2), Pval(2),~,Stats(2).stats] = ttest(yvar(:,6,3),yvar(:,7,3));%phantom med vs phantom control med
[ttestMean(3), Pval(3),~,Stats(3).stats] = ttest(yvar(:,8,3),yvar(:,9,3));%phantom high vs phantom control high

[ttestMean(4), Pval(4),~,Stats(4).stats] = ttest(yvar(:,1,3),yvar(:,4,3));%full low vs phantom low 
[ttestMean(5), Pval(5),~,Stats(5).stats] = ttest(yvar(:,2,3),yvar(:,6,3));%full med vs phantom med 
[ttestMean(6), Pval(6),~,Stats(6).stats] = ttest(yvar(:,3,3),yvar(:,8,3));%full high vs phantom high

[ttestMean(4), Pval(7),~,Stats(7).stats] = ttest(yvar(:,1,3),yvar(:,5,3));%full low vs phantom control low
[ttestMean(5), Pval(8),~,Stats(8).stats] = ttest(yvar(:,2,3),yvar(:,7,3));%full med vs phantom control med
[ttestMean(6), Pval(9),~,Stats(9).stats] = ttest(yvar(:,3,3),yvar(:,9,3));%full high vs phantom control high
...
% singleBarDotPlotSEM3(yvar,avgConditions, {'bias'}, ylab, ylims);
 plotdir = strcat('/Users/loicdaumail/Documents/Research_MacBook/Tong_Lab/Projects/motion_after_effect/anal_plots/');
mkdir(plotdir);
saveas(gcf,strcat(plotdir, sprintf('dynamicMAE_percent_bias_%s.svg', version)));
%% %% Inducer type vs contrast interaction
...
% 
[ttestMean(1), Pval(1),~,Stats(1).stats] = ttest(yvar(:,4),yvar(:,5));%phantom low vs phantom control low
[ttestMean(2), Pval(2),~,Stats(2).stats] = ttest(yvar(:,6),yvar(:,7));%phantom med vs phantom control med
[ttestMean(3), Pval(3),~,Stats(3).stats] = ttest(yvar(:,8),yvar(:,9));%phantom high vs phantom control high

[ttestMean(4), Pval(4),~,Stats(4).stats] = ttest(yvar(:,1),yvar(:,4));%full low vs phantom low 
[ttestMean(5), Pval(5),~,Stats(5).stats] = ttest(yvar(:,2),yvar(:,6));%full med vs phantom med 
[ttestMean(6), Pval(6),~,Stats(6).stats] = ttest(yvar(:,3),yvar(:,8));%full high vs phantom high

[ttestMean(4), Pval(7),~,Stats(7).stats] = ttest(yvar(:,1),yvar(:,5));%full low vs phantom control low
[ttestMean(5), Pval(8),~,Stats(8).stats] = ttest(yvar(:,2),yvar(:,7));%full med vs phantom control med
[ttestMean(6), Pval(9),~,Stats(9).stats] = ttest(yvar(:,3),yvar(:,9));%full high vs phantom control high
